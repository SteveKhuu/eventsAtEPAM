(function(window){
	...
	*/
	...
	_.extend(XF.UIElements, /** @lends XF.UIElements */ {
		...
		/**
			Reworks markup of a givven jQuery object
			@param jqObj jQuery item
		*/
		...
		/**
			A list of all the enhancements that whould be done of every jQuery object givven
			@type Object
			@private
		*/
		enhancementList : {
		...
		*/
		enhanceButton : function(button) {
			...
			var jQButton = $(button);
			if(!button || !jQButton instanceof jQuery) {
				return;
			}
		...
		*/
		enhanceTextInput : function(textInput) {
			...
			if(!textInput || !jQTextInput instanceof jQuery) {
				return;
			}
		...
		*/
		enhanceCheckboxRadio : function(chbRbInput) {
			...
			if(!chbRbInput || !jQChbRbInput instanceof jQuery) {
				return;
			}
...
/*
		...
		*/
		enhanceFieldset : function(fieldset) {
			...
			if(!fieldset || !jQFieldset instanceof jQuery) {
				return;
			}
		...
		*/
		enhanceList: function(list) {
			...
			var jQList = $(list);
			if(!list || !jQList instanceof jQuery) {
				return;
			}
			...
			anchors.each(function() {
				...
				jqAnchor.append(
					$('<div class=xf-btn-text></div>')
						.append(
							jqAnchor.contents().not('.xf-icon, .xf-count-bubble, .xf-li-thumb')
						)
				);
			});
	    ...
	    enhanceScrollable : function(scrollable) {
	        ...
	        if(!scrollable || !jQScrollable instanceof jQuery) {
	            return;
	        }
	        ...
	        var needRefreshIScroll = function(){
	            if(jQuery.contains($('#' + wrapperId)[0], this)) {
	                wrapperChanged = true;
		...
		*/
        ...
        /**
         ...
         @param buttons Array of buttons to show (jQuery objects or objects with button description for createButton() method)
        */
		...
		/**
			...
			@param jqPopup jQuery object representing popup
		*/
		...
		*/
		...
		/**
			...
			@return jQuery Notification object
		*/
	...
	*/
	var loadChildComponents = function(DOMObject) {
		$(DOMObject).find('[data-component]:visible, [data-component][data-cache=true]').each(function(ind, value) {
	...
	_.extend(XF.Component.prototype, /** @lends XF.Component.prototype */{
		...
		*/
		...
		/**
			...
			@return {String} Selector string that can be used for jQuery.find() for example
		*/
		selector : function() {
	...
	*/
	...
	_.extend(XF.Model.prototype, /** @lends XF.Model.prototype */{
		...
		*/
		...
		/**
			Settings for jQuery AJAX data request
			@type String
		*/
		...
		*/
		...
		/**
			Returns settings for jQuery AJAX data request or empty object is it is not set - override to add extra functionality
			@private
		*/
						...
						complete : function(jqXHR, textStatus) {
							...
							if(textStatus == 'success') {
								if($this.isStringData) {
									...
									$this.rawData = $.parseJSON(jqXHR.responseText);
								}
							} else {
								if($this.isStringData) {
									$this.rawData = {};
...
//			var reverseClass = this.animationReverseClass;
	...
	/* adding touchable functionality as jQuery plugin */
	...
	*/
	...
	_.extend(XF.DeviceClass.prototype, /** @lends XF.DeviceClass.prototype */{
		...
		*/
		...
		/**
			Returns viewport jQuery object
			@return JQuery
		*/
